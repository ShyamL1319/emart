                        ...
                        this.cartManagementService.getCustomerCart(userToken).subscribe(data => {
                          ...
                          if( data && data.hasOwnProperty('data') && data.data.length > 0 ) {  
                            if (isPlatformBrowser(this.platformId)) {
                              ...
                              /************** update GST and no of Items***/
                              let cost = 0, GST = 0, grandTotal = 0 ;
                              Object.keys(data.data[0].cart).map((key, value) => {
                                 cost += data.data[0].cart[value].price;
                              });
                              /****************** ends ******************/
                              GST = usersModule.calculateVAT(cost);
                              grandTotal = cost + GST ;
                              if (isPlatformBrowser(this.platformId))  {
...
/******************** ends  *************/
  ...
  isAdmin(){
    ...
    if(data) {
      if(data.session === 'loggedIn' && data.mode === 'registered'&& data.email === 'shyam.admin@gmail.com') {
           this.admin = true;
       }
    }
  }
