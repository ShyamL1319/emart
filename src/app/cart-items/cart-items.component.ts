...
export class CartItemsComponent implements OnInit {
  ...
  private title:Title, private meta: Meta,  @Inject(PLATFORM_ID) private platformId: Object) {
    ...
    title.setTitle('E-Mart: products in cart');

    // Sets the <meta> tag for the page
    meta.addTags([
      { name: 'author', content: 'E-Mart' },
      { name: 'description', content: 'E-Mart: see the products you added them to the cart.' },
    ]);
   }
  ...
  calculateTotalCost() {
    ...
    if (this.itemsInCart && this.itemsInCart.length !== 0) {
      ...
      this.calculatedGST = usersModule.calculateGST(this.cost);
      this.grandTotal = this.cost + this.calculatedGST;
      if ( isPlatformBrowser(this.platformId) ) { 
      localStorage.setItem('grandTotal', (String)(this.grandTotal));
      localStorage.setItem('GST', (String)(this.calculatedGST)); }
      this.noOfItems = this.itemsInCart.length;
    }else {
     setTimeout(() => {
      this.noItems = true;
  ...
  {
    if(cost)
     this.calculatedGST = this.GST*cost
     return this.calculatedGST;
  }
  /***************ends ***************/
  ngOnInit() {
    this.attach();
